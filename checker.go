...
// isCopyleft checks if license text contains any copyleft keyword.
func isCopyleft(license string) bool{
	...
	for _,kw:=range copyleftLicenses{if strings.Contains(license,kw){return true}}
	return false
}

// findFile searches the directory tree for target.
func findFile(root,target string)string{
	var found string
	filepath.WalkDir(root,func(path string,d fs.DirEntry,err error)error{
		if err!=nil{return nil}
		if d.Name()==target{found=path;return filepath.SkipDir}
		return nil
	})
	return found
}

// NodeDependency for Node.js packages.
...
func resolveNodeDependency(pkgName,version string,visited map[string]bool)(*NodeDependency,error){
	...
	if ver==""{
		if dt,ok:=data["dist-tags"].(map[string]interface{});ok{
			if latest,ok:=dt["latest"].(string);ok{ver=latest}}}
	}
	versions,ok:=data["versions"].(map[string]interface{})
	if !ok{return nil,fmt.Errorf("versions not found for %s",pkgName)}
	...
	if !ok{
		if dt,ok:=data["dist-tags"].(map[string]interface{});ok{
			if latest,ok:=dt["latest"].(string);ok{
				...
				if ok{ver=latest}else{return nil,fmt.Errorf("version data not found for %s",pkgName)}
			}
		}
	}
	lic:="Unknown"
	if l,ok:=versionData["license"].(string);ok{lic=l}else if l,ok:=versionData["license"].(map[string]interface{});ok{
		if t,ok:=l["type"].(string);ok{lic=t}}
	...
	if deps,ok:=versionData["dependencies"].(map[string]interface{});ok{
		for dep,depVerRange:=range deps{
			...
			if !ok{log.Printf("Warning:invalid version for %s of %s",dep,pkgName);continue}
			tdep,err:=resolveNodeDependency(dep,depVer,visited)
			if err!=nil{log.Printf("Error resolving transitive dependency %s of %s:%v",dep,pkgName,err);continue}
			if tdep!=nil{trans=append(trans,tdep)}
		}
	}
	return &NodeDependency{
		Name:pkgName,Version:ver,License:lic,Details:details,
		Copyleft:isCopyleft(lic),Transitive:trans,Language:"node"},nil
}
...
func parseNodeDependencies(filePath string)([]*NodeDependency,error){
	...
	if err!=nil{return nil,err}
	var data map[string]interface{}
	if err=json.Unmarshal(file,&data);err!=nil{return nil,err}
	deps,ok:=data["dependencies"].(map[string]interface{})
	if !ok{return nil,fmt.Errorf("no dependencies found in package.json")}
	...
	for name,ver:=range deps{
		...
		if err==nil&&nodeDep!=nil{results=append(results,nodeDep)}
	}
	return results,nil
}

// PythonDependency for Python packages.
...
func resolvePythonDependency(pkgName,version string,visited map[string]bool)(*PythonDependency,error){
	...
	if err!=nil{return nil,err}
	defer resp.Body.Close()
	if resp.StatusCode!=http.StatusOK{return nil,fmt.Errorf("PyPI returned status:%s",resp.Status)}
	var data map[string]interface{}
	if err=json.NewDecoder(resp.Body).Decode(&data);err!=nil{return nil,err}
	info,ok:=data["info"].(map[string]interface{})
	if !ok{return nil,fmt.Errorf("info not found for %s",pkgName)}
	ver:=version
	if ver==""{
		if rel,ok:=info["version"].(string);ok{ver=rel}else{return nil,fmt.Errorf("version not found for %s",pkgName)}}
	releases,ok:=data["releases"].(map[string]interface{})
	if !ok{return nil,fmt.Errorf("releases not found for %s",pkgName)}
	if _,ok:=releases[ver].([]interface{});!ok{
		if rel,ok:=info["version"].(string);ok{ver=rel}else{return nil,fmt.Errorf("version %s not found for %s",ver,pkgName)}}
	lic:="Unknown"
	if l,ok:=info["license"].(string);ok{lic=l}
	details:=url
	return &PythonDependency{
		Name:pkgName,Version:ver,License:lic,Details:details,
		Copyleft:isCopyleft(lic),Language:"python"},nil
}

func parsePythonDependencies(filePath string)([]*PythonDependency,error){
	file,err:=os.Open(filePath)
	if err!=nil{return nil,fmt.Errorf("could not open:%w",err)}
	defer file.Close()
	...
	for _,r:=range reqs{
		...
		go func(r requirement){
			defer wg.Done()
			dep,err:=resolvePythonDependency(r.name,r.version,map[string]bool{})
			if err!=nil{errChan<-fmt.Errorf("resolving %s@%s:%w",r.name,r.version,err);return}
			if dep!=nil{depChan<-*dep}
		}(r)
	}
	go func(){wg.Wait();close(depChan);close(errChan)}()
	for d:=range depChan{resolved=append(resolved,&d)}
	for e:=range errChan{log.Println(e)}
	return resolved,nil
}
...
func parseRequirements(r io.Reader)([]requirement,error){
	...
	for _,line:=range lines{
		...
		if len(parts)!=2{
			parts=strings.Split(line,">=")
			if len(parts)!=2{log.Printf("Warning: invalid line:%s",line);continue}
		}
...
func flattenAllNodeDeps(nds []*NodeDependency,parent string)[]FlatDep{
	...
	for _,nd:=range nds{
		...
		if len(nd.Transitive)>0{
			flats=append(flats,flattenAllNodeDeps(nd.Transitive,nd.Name)...)
		}
	}
	return flats
}
...
func flattenAllPythonDeps(pds []*PythonDependency,parent string)[]FlatDep{
	...
	for _,pd:=range pds{
		...
		if len(pd.Transitive)>0{
			flats=append(flats,flattenAllPythonDeps(pd.Transitive,pd.Name)...)
		}
	}
	return flats
}
...
func dependencyTreeJSONFinal(nodeDeps []*NodeDependency,pythonDeps []*PythonDependency)(string,string,error){
	dummyNode:=map[string]interface{}{
		"Name":"Node.js Dependencies","Version":"",
		"Transitive":nodeDeps}
	dummyPython:=map[string]interface{}{
		"Name":"Python Dependencies","Version":"",
		"Transitive":pythonDeps}
	nodeJSON,err:=json.MarshalIndent(dummyNode,"","  ")
	if err!=nil{return"","",err}
	pythonJSON,err:=json.MarshalIndent(dummyPython,"","  ")
	if err!=nil{return"","",err}
	return string(nodeJSON),string(pythonJSON),nil
}
...
<h2>Dependencies Table</h2><table><tr>
<th>Dependency</th><th>Version</th><th>License</th>
<th>Parent</th><th>Language</th><th>Details</th>
</tr>{{range .FlatDeps}}<tr><td>{{.Name}}</td><td>{{.Version}}</td>
<td class="{{if eq .License "Unknown"}}unknown{{else if isCopyleft .License}}copyleft{{else}}non-copyleft{{end}}">
{{.License}}</td><td>{{.Parent}}</td><td>{{.Language}}</td>
<td><a href="{{.Details}}" target="_blank">View</a></td></tr>{{end}}</table>
<h2>Dependency Graph Visualization</h2>
<p>The graphs below represent the dependency trees for Node.js and Python.</p>
<h3>Node.js Dependency Tree</h3><div id="nodeGraph"></div>
<h3>Python Dependency Tree</h3><div id="pythonGraph"></div>
...
width=660-margin.left-margin.right,height=500-margin.top-margin.bottom;
...
.attr("fill","#fff").attr("stroke","steelblue")
.attr("stroke-width","3");
...
func main(){
	...
	if nodeFile!=""{
		...
		if err!=nil{log.Println("Error parsing Node.js:",err)}else{nodeDeps=nd}
	}

	pythonFile:=findFile(".","requirements.txt")
	if pythonFile==""{pythonFile=findFile(".","requirement.txt")}
	var pythonDeps []*PythonDependency
	if pythonFile!=""{
		pd,err:=parsePythonDependencies(pythonFile)
		if err!=nil{log.Println("Error parsing Python:",err)}else{pythonDeps=pd}
	}

	flatNode:=flattenAllNodeDeps(nodeDeps,"Direct")
	flatPython:=flattenAllPythonDeps(pythonDeps,"Direct")
	flatDeps:=append(flatNode,flatPython...)

	totalCopyleft:=0
	for _,dep:=range flatDeps{if isCopyleft(dep.License){totalCopyleft++}}
	summary:=fmt.Sprintf("%d direct Node.js dependencies, %d direct Python dependencies. Total copyleft dependencies: %d.",
		len(nodeDeps),len(pythonDeps),totalCopyleft)
